...
function addon:LoadSpells()                                                                                     -- DIFFERENCE Retail Code
    ...
    for i = 1, GetNumSpellTabs() do
        local name, _, offset, numSlots, isGuild = GetSpellTabInfo(i)

        -- Ensure the tab has a name and is not a guild tab
        if name and not isGuild then
            addon.spells[name] = {}

            -- Loop through all slots in the tab
            for j = offset + 1, offset + numSlots do
                ...
                -- Add only active spells
                if spellName and spellType == "SPELL" then
